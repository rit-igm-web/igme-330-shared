...
4) Delete the contents of the "babble" arrays and move that data to an external JSON file named **babble-data.json**
    ...
    - Important: structure your JSON data to be a simple as possible - it just needs to contain the 3 arrays of babble - so it can look very similar to the **simple-pet-names.json** file in "Ajax 4" (link in Week 3 Content area when we cover it)
    - Use `fetch()` to load the  **data/babble-data.json** file - and assign the data to the `words1`, `words2` and `words3` arrays
      - You will probably have a `loadBabble()` function that will be called just ONCE, *after* the page has loaded
        - `loadBabble()` will use `fetch()` to download the JSON file
        - `loadBabble()` will use `.then()` to handle the response and parse the JSON
        - `loadBabble()` will initialize the values of the "words" arrays with the parsed data
        - `loadBabble()` will then initialize the button click events
        - `loadBabble()` will also call another function that displays some initial "start up" babble
